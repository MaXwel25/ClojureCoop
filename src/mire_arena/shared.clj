...
(def initial-state
  {:players {}      ;; player-id -> {:id :x :y :hp :score :last-shot :dead :dead-since :speed-buff :damage-buff}
...
(defn collides? [x1 y1 size1 x2 y2 size2]
  ;; проверяем, что все параметры не nil
  (when (and x1 y1 x2 y2 size1 size2)
...
(defn collides-with-player? [x y size players]
  (some (fn [[_ player]]
          (and (not (:dead player))
               (let [entity-size (if (= (:id player) "boss") boss-size player-size)] 
                 (collides? x y size
                            (:x player) (:y player) entity-size))))
        players))
...
(defn create-bullet [bullet-id x y dx dy owner]
  {:id bullet-id
   :x x :y y
   :dx dx :dy dy
   :owner owner  ; owner остается строкой (UUID или "boss")
   :created-at (System/currentTimeMillis)})

(defn create-player [player-id]  ;; player-id параметр
  (let [pos (random-spawn-position)]
    (merge pos
           {:id player-id  ;; включаем id
            :hp max-hp
            :score 0
            :last-shot 0
            :dead false
...
(defn create-boss []
  {:id "boss"  ; босс всегда имеет id "boss" (строка)
   :x 400 :y 300
   :hp boss-max-hp
   :score 0
   :last-shot 0
...
(defn prepare-game-state-for-broadcast [state]
  {:type "state"
   ...
   :bullets (vec (vals (:bullets state)))
   :bonuses (vec (vals (:bonuses state)))})
